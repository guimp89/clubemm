...
The public API lets you drive your Clube Melissa CRM instance from your own
...
```
Authorization: Bearer melissacrm_live_xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
```
...
3. Copy the key. **The full key is shown exactly once.** Clube Melissa CRM
...
curl https://your-crm.example.com/api/v1/me \
  -H "Authorization: Bearer melissacrm_live_xxx"
```
...
Rather than polling, register an endpoint and Clube Melissa CRM will POST to it when
...
- `POST /api/v1/webhooks` — register `{ "url": "https://…", "events": ["message.received"] }`. `url` must be `https://`. **The response includes `secret` exactly once** — store it to verify signatures; Clube Melissa CRM keeps only an encrypted copy.
...
curl -X POST https://your-crm.example.com/api/v1/webhooks \
  -H "Authorization: Bearer melissacrm_live_xxx" \
  -H "Content-Type: application/json" \
  -d '{ "url": "https://example.com/hooks/crm", "events": ["message.received"] }'
...
Headers: `X-MelissaCRM-Event`, `X-MelissaCRM-Webhook-Id`, and `X-MelissaCRM-Signature`.

### Verifying the signature

`X-MelissaCRM-Signature: t=<unix_seconds>,v1=<hex>` where `v1 =
...
covers messages Clube Melissa CRM stores (inbox + API sends), not broadcast-only
...
broadcasts, and outbound webhooks. Future ideas (deals/pipelines,
